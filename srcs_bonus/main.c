...
/*   Updated: 2024/09/16 13:53:14 by marvin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	load_textures(t_cube *cube)
{
	...
	load_img(cube, 1, "./assets/psycho2.xpm", cube->assets_ennemies);
	load_img(cube, 2, "./assets/psycho3.xpm", cube->assets_ennemies);
	load_img(cube, 1, "./assets/gameover.xpm", cube->screen);
	load_img(cube, 2, "./assets/screamer.xpm", cube->screen);
}
...
void	init_cube(t_cube *cube, t_data *img, t_key *key)
{
	...
	cube->plane_y = 0;
	cube->size_asst_e = 2;
	cube->last_time = get_current_time();
	cube->sound_oldt = get_current_time();
	cube->i_texture = 0;
	cube->gameover = 0;
}
...
int	main(int ac, char **av)
{
	...
	load_img(&cube, 0, "./assets/load.xpm", cube.screen);
	mlx_put_image_to_window(cube.mlx_ptr,
		cube.win_ptr, cube.screen[0].img, 0, 0);
	playsound("forload", 0, 0, 0);
